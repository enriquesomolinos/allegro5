...
static int vita_sys_init(void);
static void vita_sys_exit(void);
BITMAP * vita_create_bitmap_ex(int color_depth, int width, int height, int do_scaling);
static BITMAP * vita_create_bitmap(int color_depth, int width, int height);
static void vita_get_gfx_safe_mode(int *driver, struct GFX_MODE *mode);
static int align_to_16(int width, int color_depth);
static int next_power_of_2(int width);



SYSTEM_DRIVER system_vita =
{
   SYSTEM_VITA,
   empty_string,
   empty_string,
   ...
   vita_create_bitmap,  /* AL_METHOD(struct BITMAP *, create_bitmap, (int color_depth, int width, int height)); */
   ...
   vita_get_gfx_safe_mode,  /*AL_METHOD(void, get_gfx_safe_mode, (int *driver, struct GFX_MODE *mode));*/
...
/* vita_sys_init:
 *  Initializes the VITA system driver.
 */
static int vita_sys_init(void)
{
...
/* vita_create_bitmap_ex:
 *  Creates a RAM bitmap with proper VITA pitch.
 */
BITMAP * vita_create_bitmap_ex(int color_depth, int width, int height, int do_scaling)
{
   GFX_VTABLE *vtable;
...
{
   int do_scaling = TRUE;
   return vita_create_bitmap_ex(color_depth, width, height, do_scaling);
}



/* vita_get_gfx_safe_mode:
